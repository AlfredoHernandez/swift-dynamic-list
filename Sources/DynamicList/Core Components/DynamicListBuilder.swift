...
// MARK: - Sectioned DynamicList Builder
...
/// in a NavigationStack. It's not meant to be used directly by consumers.
private struct SectionedDynamicListWrapper<Item: Identifiable & Hashable>: View {
...
// MARK: - SectionedDynamicList Content
...
/// the `buildWithoutNavigation()` method.
private struct SectionedDynamicListContent<Item: Identifiable & Hashable>: View {
    @State private var viewModel: SectionedDynamicListViewModel<Item>
    private let rowContent: (Item) -> AnyView
    private let detailContent: (Item) -> AnyView
    private let errorContent: ((Error) -> AnyView)?
